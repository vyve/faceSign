...
public class AutoCheck {
    ...
    private AutoCheck innerCheck() {
        boolean isOnlineSdk = TtsMode.ONLINE.equals(TtsMode.MIX);
        ...
        checks.put("检查AppId AppKey SecretKey",
                new AppInfoCheck(SpeechSynthesisAuthMessage.getInstance(context).getAppId(),
                        SpeechSynthesisAuthMessage.getInstance(context).getAppKey(),
                        SpeechSynthesisAuthMessage.getInstance(context).getSecretKey()));
        checks.put("检查包名", new ApplicationIdCheck(context, SpeechSynthesisAuthMessage.getInstance(context).getAppId()));
