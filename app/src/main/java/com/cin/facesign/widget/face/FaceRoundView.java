...
public class FaceRoundView extends View {
    ...
    public FaceRoundView(Context context, AttributeSet attrs) {
        ...
        float pathSpace = ConvertUtils.dp2px(PATH_SPACE);
        float pathSmallSpace = ConvertUtils.dp2px(PATH_SMALL_SPACE);
        float pathWidth = ConvertUtils.dp2px(PATH_WIDTH);
...
//        }
    ...
    public static Rect getPreviewDetectRect(int w, int pw, int ph) {
        float round = (w / 2f) - ((w / 2f) * WIDTH_SPACE_RATIO);
        float x = pw / 2f;
        float y = (ph / 2f) - ((ph / 2f) * HEIGHT_RATIO);
        float r = Math.min((pw / 2f), round);
        float hr = r + (r * HEIGHT_EXT_RATIO);
        //        Log.e(TAG, "FaceRoundView getPreviewDetectRect " + pw + "-" + ph + "-" + rect.toString());
        return new Rect((int) (x - r),
                (int) (y - hr),
                (int) (x + r),
                (int) (y + hr));
    }
